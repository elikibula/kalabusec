...
from django.db.models import Q, Count, Prefetch, Avg, OuterRef, Subquery
...
class CourseOwnerMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        ...
        return user.is_teacher and course.teacher == user
...
@login_required
@require_POST
def teacher_unenroll_student(request, pk):
    """Allow teacher or school admin to unenroll a student"""
...
def course_analysis_export_csv(request, pk):
    ...
    writer.writerow([
        'Student Name', 'Username', 'Status', 'Lessons Completed',
        'Total Lessons', 'Progress %', 'Assignments Submitted',
        'Assignments Graded', 'Average Assignment Score',
        'Quiz Attempts', 'Average Quiz Score', 'Certificate Issued'
    ])
    ...
    for enr in enrollments:
        ...
        avg_assignment_score = submissions.filter(status='graded').aggregate(Avg('score'))['score__avg'] or 0

        quiz_attempts = QuizAttempt.objects.filter(
            student=student, quiz__course=course, submitted_at__isnull=False
        )
        ...
        writer.writerow([
            ...
            progress_pct,
            submissions.count(),
            submissions.filter(status='graded').count(),
            round(avg_assignment_score, 2),
            quiz_attempts.count(),
...
class CourseDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_student:
            ...
            roster = Enrollment.objects.filter(course=course).select_related('student')
            ...
            ctx['student_progress'] = [{
                ...
                'total': total_lessons,
                'pct': round((per_student.get(enr.student_id, 0) / total_lessons * 100)
...
def enroll_course(request, pk):
    ...
    if created:
        if initial_status == 'active':
            ...
            _notify(request.user, f'Your enrolment request for {course.title} has been submitted.', 'enrolment_request')
            if course.teacher:
                _notify(course.teacher, f'{_display_name(request.user)} wants to join {course.title}.', 'enrolment_request')
...
# Enrolment management views (approve/reject)
@login_required
...
def reject_enrolment(request, enrollment_pk):
    ...
    _notify(enr.student, msg, 'enrolment_rejected', link=reverse('courses:detail', kwargs={'pk': course.pk}))
...
def student_roster(request, pk):
    ...
    roster_data = [{
        ...
        'completed': per_student.get(enr.student_id, 0),
        'total': total_lessons,
        'pct': round((per_student.get(enr.student_id, 0) / total_lessons * 100) if total_lessons > 0 else 0, 1),
    } for enr in enrollments]
...
class LessonDetailView(LoginRequiredMixin, DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if user.is_authenticated:
            ...
            if user.is_student and lesson.release_at and timezone.now() < lesson.release_at:
                messages.warning(request, f'"{lesson.title}" will unlock on {timezone.localtime(lesson.release_at).strftime("%d %b %Y %H:%M")}.')
                return redirect('courses:detail', pk=course.pk)

            if user.is_student and is_enrolled and not lesson.is_accessible_to(user):
                messages.warning(request, f'Complete "{lesson.prerequisite.title}" before accessing this lesson.')
                return redirect('courses:detail', pk=course.pk)
    ...
    def get_context_data(self, **kwargs):
        ...
        # Navigation
        siblings = list(_student_visible_lessons_queryset(lesson.module.course).filter(module=lesson.module).order_by('order'))
...
def complete_lesson(request, pk):
    ...
    course = lesson.module.course
    if not _is_enrolled_active(user, course):
        messages.error(request, 'You must be enrolled.')
        return redirect('courses:detail', pk=course.pk)

    if not lesson.is_accessible_to(user):
    ...
    if created:
        ...
        if total > 0 and completed == total:
            cert = _maybe_issue_certificate(user, course)
            if cert:
                messages.success(request, f'You completed {course.title}! Your certificate is ready.')
    else:
    ...
    with transaction.atomic():
        ...
        for module in course.modules.prefetch_related('lessons__files').order_by('order'):
            ...
            for lesson in module.lessons.order_by('order'):
                ...
                for lf in lesson.files.all():
                    LessonFile.objects.create(lesson=new_lesson, file=lf.file.name, label=lf.label)

        # Set prerequisites
        for old_pk, new_lesson in lesson_map.items():
            old_lesson = Lesson.objects.get(pk=old_pk)
        ...
        # Duplicate quizzes (simplified - add full quiz duplication if needed)
        for quiz in course.quizzes.prefetch_related('questions').order_by('-available_from'):
...
# Module & Lesson CRUD views (kept mostly original)
...
# ... (ModuleUpdateView, ModuleDeleteView, LessonCreateView, LessonUpdateView, LessonDeleteView remain the same as original)
...
def view_certificate(request, pk):
    cert = get_object_or_404(CourseCompletionCertificate, pk=pk, student=request.user)
    return render(request, 'courses/certificate.html', {'cert': cert})


@login_required
def my_certificates(request):
    certs = CourseCompletionCertificate.objects.filter(student=request.user).select_related('course')
